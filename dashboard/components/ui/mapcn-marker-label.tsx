...
function MapMarker({ longitude, latitude, children, onClick, onMouseEnter, onMouseLeave, onDragStart, onDrag, onDragEnd, draggable = false, ...markerOptions }: MapMarkerProps) {
  ...
  useEffect(() => {
    if (!map) return;
    marker.addTo(map);
    // Braced on purpose: Marker#remove() is chainable and returns the marker, and
    // an effect cleanup that returns a value is a type error (React reads a
    // returned function as the NEXT cleanup). Upstream ships it unbraced.
    return () => {
      marker.remove();
    };
  }, [map, marker]);
